...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # 3. Escalar y desplazar (Scale and shift)
        out = gamma * x_hat + beta
        # 4. Actualizar las medias y varianzas móviles (running averages)
        running_mean = momentum * running_mean + (1.0 - momentum) * sample_mean
        running_var = momentum * running_var + (1.0 - momentum) * sample_var
        # 5. Guardar variables en caché para el backward pass
        cache = (x_hat, gamma, x_minus_mean, std, eps)
...
def svm_loss(x, y):
    ...
    dx = np.zeros_like(x)
    dx[margins > 0] = 1.0
    row_sums = np.sum(dx, axis=1)
    dx[np.arange(N), y] -= row_sums
    dx /= N
    # ### END CODE HERE ###
    ###########################################################################
    #                             END OF YOUR CODE                            #
