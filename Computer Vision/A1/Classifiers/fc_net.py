...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(1, len(dims)):
            ...
            # gamma/beta solo para capas hidden (no la última) y solo si hay norm
            if i < len(dims) - 1 and self.normalization in ("batchnorm", "layernorm"):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.normalization == "batchnorm":
                gammai, betai = self.params[f"gamma{i}"], self.params[f"beta{i}"]
                out, fc_cache = affine_forward(out, Wi, bi)
                out, bn_cache = batchnorm_forward(out, gammai, betai, self.bn_params[i - 1])
                ...
                gammai, betai = self.params[f"gamma{i}"], self.params[f"beta{i}"]
                out, fc_cache = affine_forward(out, Wi, bi)
                out, ln_cache = layernorm_forward(out, gammai, betai, self.bn_params[i - 1])
                out, relu_cache = relu_forward(out)
